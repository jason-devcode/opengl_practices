...
#define WIDTH 1280
#define HEIGHT 1024
#define TITLE "12 - ImGui Integration"
...
GLuint build_vao()
{
  ...
  Vertex vertices[] = {
      {.pos = {-0.5f,  0.5f, 0.0f}, .color = {1.0f, 0.0f, 0.0f}, .tx = { 0.0, 0.0 } },
      {.pos = { 0.5f,  0.5f, 0.0f}, .color = {0.0f, 1.0f, 0.0f}, .tx = { 1.0, 0.0 } },
      {.pos = { 0.5f, -0.5f, 0.0f}, .color = {0.0f, 0.0f, 1.0f}, .tx = { 1.0, 1.0 } },
      {.pos = {-0.5f, -0.5f, 0.0f}, .color = {1.0f, 1.0f, 1.0f}, .tx = { 0.0, 1.0 } }};
...
Texture gen_chess_texture( int w, int h ) {
  LOG_INFO( "Generating chessboard texture...");

  uint32_t* data = new uint32_t[ w * h ];

  LOG_INFO("Chessboard Texture: width=%d height=%d", w, h);

  if( !data ) {
    LOG_ERROR( "Failed to generate chessboard texture!" );
    return { 0, 0, 0, NULL, 0, GL_FALSE };
  }
  ...
  for( int Y = 0; Y < h; Y++ ) {
    for( int X = 0; X < w; X++ ) {
      if( (X & 32) ^ (Y & 32)) data[ Y * w + X ] = 0xFFFFFFFF;
      else data[ Y * w + X ] = 0xFF000000;
    }
  }
  ...
  LOG_SUCCESS( "Chessboard texture generated successfully!\n" );

  return (Texture) {
    .width = w, .height = h, .nrChannels = 4,
    .data = (uint8_t*)data,
    .obj = tx_obj,
    .success = GL_TRUE
  };
...
void render_loop( GLFWwindow* window, int initial_width, int initial_height ) {
  ...
  Texture chess_tx = gen_chess_texture( 1024, 1024 );
  ...
  while ( !glfwWindowShouldClose( window ) )
  {
    ...
    glBindTexture( GL_TEXTURE_2D, chess_tx.obj );
