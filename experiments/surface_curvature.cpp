    ...
    for( int j = 0; j < (rows + 1); j++ ) {
      for( int i = 0; i < (cols + 1); i++ ) {
        ...
        vertices[ vertexIndex ] = (Vertex){
          ...
          front,
          vec2( (float) i / (float) (cols + 1), (float) j / (float) (rows + 1) )
        };
...
void main() {
  vColor = aColor;
  vec3 sphrPos = aPos * (1.0f - spherization) + normalize(aPos) * spherization;
...
vec3 camera_pos = vec3(0.0f, 0.0f, 4.0f);
...
void render_loop( GLFWwindow* window ) {
  ...
  Mesh cube = gen_cube( 16, 16, 0.0f );
  Mesh sphere = gen_cube( 32, 32, spherization );
  
  GLuint common_shader = process_shaders();
  ...
  while ( !glfwWindowShouldClose( window ) ) {
    ...
    glDrawElements( GL_TRIANGLES, sphere.numTriangles * 3, GL_UNSIGNED_INT, 0 );
